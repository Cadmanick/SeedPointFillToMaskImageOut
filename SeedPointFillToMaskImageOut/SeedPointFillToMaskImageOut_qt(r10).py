...
class FloodFillApp(QMainWindow):
    ...
    def choose_gap_kernel_descending(self, start_k: int, perimeter_jump_ratio: float = 1.30, min_k: int = 1):
        ...
        for k in range(start_k, min_k - 1, -1):
            mask_k, ctr_k = self._compute_flood_mask_with_gap_edge_walls(k)
    ...
    def place_brown_line_at_bottleneck(self, narrowest_pt, boundary_contour, half_length=50):  # ✅ Added self
        """Draw perpendicular brown line at narrowest point."""
        print(f"[Auto Barrier] Placing brown line at {narrowest_pt}")  # ✅ Now executes
    ...
    def flood_fill_and_show_mask_contours_modular(self):
        ...
        # NEW: envelope clamp settings (prevents external expansion)
        envelope_dilate_factor = 1  # scales how much expansion is allowed beyond baseline
        min_envelope_dilate_px = 3  # ensures some allowance on small gaps

        # TUNING: cap growth even if still inside envelope
        inside_growth_threshold = 1.05  # allow 10% growth inside envelope

        # TUNING: ignore tiny leak noise
        leak_blob_min_px = 2000
        ...
        baseline_area = None
        baseline_mask_u8 = None  # NEW: hold baseline mask for envelope generation
        ...
        for outer_iter in range(max_outer_iterations):
            ...
            mask_at_optimal, _ = self._compute_flood_mask_with_gap_edge_walls(optimal_gap_k)
            ...
            # --- Establish baseline (tight) mask once ---
            if baseline_area is None:
                baseline_k = max(current_gap_px, optimal_gap_k + 4)
                baseline_mask, _ = self._compute_flood_mask_with_gap_edge_walls(baseline_k)
                baseline_area_candidate = int(np.count_nonzero(baseline_mask)) if baseline_mask is not None else 0

                if baseline_mask is not None and baseline_area_candidate > 0:
                    baseline_mask_u8 = baseline_mask
                    baseline_area = baseline_area_candidate
                    print(f"[Baseline] Established baseline area={baseline_area} at baseline_k={baseline_k}")
                else:
                    baseline_mask_u8 = mask_at_optimal
                    baseline_area = area_at_optimal
                    print(f"[Baseline] Fallback baseline area={baseline_area} (from optimal)")

            # --- Build envelope from baseline (prevents external expansion) ---
            envelope_dilate_px = max(min_envelope_dilate_px, int(current_gap_px * envelope_dilate_factor))
            se_env = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * envelope_dilate_px + 1, 2 * envelope_dilate_px + 1))
            allowed_envelope = cv2.dilate(baseline_mask_u8, se_env, iterations=1)

            # --- “External expansion leak” check ---
            # Any fill outside allowed envelope is a leak
            outside_envelope = cv2.bitwise_and(mask_at_optimal, cv2.bitwise_not(allowed_envelope))

            # Merge leak pixels into stable blobs (avoids speckle sensitivity)
            leak_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7))
            outside_envelope_blob = cv2.morphologyEx(outside_envelope, cv2.MORPH_CLOSE, leak_kernel, iterations=1)
            outside_px = int(np.count_nonzero(outside_envelope_blob))

            # Existing threshold check (kept)
            area_is_reasonable = area_at_optimal <= baseline_area * area_growth_threshold

            # NEW: cap growth even if still inside envelope
            inside_cap_ok = area_at_optimal <= int(baseline_area * inside_growth_threshold)

            # NEW: external expansion is only considered real if it forms a big enough blob
            expansion_is_allowed = outside_px < leak_blob_min_px

            min_acceptable_gap = max(2, int(current_gap_px * 0.6))
            gap_is_acceptable = optimal_gap_k >= min_acceptable_gap

            print(f"[Validation] gap_k={optimal_gap_k} >= min_gap{min_acceptable_gap}? {gap_is_acceptable}")
            print(f"[Validation] OptimalArea={area_at_optimal} <= BaselineAreaXthreshold={baseline_area * area_growth_threshold:.0f}? {area_is_reasonable}")
            print(f"[Validation] OutsideEnvelopePx={outside_px} (0=OK) => expansion_is_allowed={expansion_is_allowed}")

            barriers_placed_this_iter = 0

            # === Leak handling: barriers first, then reduce gap ===
            if (not area_is_reasonable) or (not inside_cap_ok) or (not expansion_is_allowed):
                if not expansion_is_allowed:
                    print(f"+++[ENVELOPE LEAK]+++ {outside_px} px outside allowed envelope - REJECTING")
                else:
                    print(f"+++[AREA LEAK]+++ Area grew {area_at_optimal / baseline_area:.1f}x - REJECTING")

                if total_barriers_added < max_total_barriers:
                    # Reference mask intended to be tighter than mask_at_optimal
                    ref_k = max(current_gap_px, optimal_gap_k + 3)
                    ref_mask, _ = self._compute_flood_mask_with_gap_edge_walls(ref_k)

                    if ref_mask is not None and np.count_nonzero(ref_mask) > 0:
                        # Leak region = new pixels beyond the tighter reference
                        leaked_region = cv2.bitwise_and(mask_at_optimal, cv2.bitwise_not(ref_mask))
                        leak_px = int(np.count_nonzero(leaked_region))

                        # Also treat envelope overflow as a leak region if it exists (stronger signal)
                        if outside_px >= leak_blob_min_px:
                            leaked_region = outside_envelope_blob
                            leak_px = outside_px

                        if leak_px > max(500, int(baseline_area * 0.02)):
                            for barrier_attempt in range(max_auto_barriers_per_iteration):
                                narrowest_pt = self.find_narrowest_point(leaked_region, ref_mask)
                                if not narrowest_pt:
                                    print("No narrowest point found")
                                    break

                                contours, _ = cv2.findContours(ref_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
                                if not contours:
                                    print("No contours in ref_mask")
                                    break

                                boundary = max(contours, key=cv2.contourArea).reshape(-1, 2)

                                pt1, pt2 = self.place_brown_line_at_bottleneck(
                                    narrowest_pt,
                                    boundary,
                                    half_length=current_gap_px / 2
                                )

                                if pt1 is None or pt2 is None:
                                    print("Failed to place barrier geometry")
                                    break
                                ...
                                # Re-run at the same gap to test effect
                                test_mask, _ = self._compute_flood_mask_with_gap_edge_walls(optimal_gap_k)
                                if test_mask is None:
                                    print("[Barrier Test] No region after barrier")
                                    break

                                test_outside = cv2.bitwise_and(test_mask, cv2.bitwise_not(allowed_envelope))
                                test_outside_px = int(np.count_nonzero(test_outside))
                                test_area = int(np.count_nonzero(test_mask))

                                if test_outside_px == 0 and test_area <= baseline_area * area_growth_threshold:
                                    mask_at_optimal = test_mask
                                    area_at_optimal = test_area
                                    area_is_reasonable = True
                                    expansion_is_allowed = True
                                    print(f"[Barrier Effective] Area={area_at_optimal}, OutsideEnvelopePx={test_outside_px}")
                                    break

                                # keep trying more barriers for the remaining leak region
                                leaked_region = cv2.bitwise_and(test_mask, cv2.bitwise_not(ref_mask))

            # If barriers were placed, retry same gap to keep searching internally
            if barriers_placed_this_iter > 0:
                print(f"[Recovery] {barriers_placed_this_iter} barriers placed; retrying at gap_px={current_gap_px}")
                continue

            # If still leaking, reduce gap and keep searching internal defects
            if (not area_is_reasonable) or (not expansion_is_allowed):
                current_gap_px = max(min_gap_px, current_gap_px - 1)
                print(f"[Gap Reduction] Reducing to gap_px={current_gap_px} after unresolved leak")
                continue
        ...
        if best_mask is None:
            if last_valid_mask is not None:
                ...
                best_mask, _ = self._compute_flood_mask_with_gap_edge_walls(1)
                self.aggressiveness_slider['slider'].setValue(base_aggressiveness)
        ...
        # === Integrate with existing mask (bridging logic) ===
...
# )
    ...
    def _compute_flood_mask_with_gap_edge_walls(self, gap_k: int):
        """
        Flood fill constrained by an edge-derived wall mask (no brown lines required).
        Returns: (mask_u8, outer_contour or None)
        """
        if self.original_image is None or not self.seed_points:
            return None, None
        ...
        brightness_value = self.brightness_slider['slider'].value()
        beta = brightness_value
        src = cv2.convertScaleAbs(img, alpha=alpha, beta=beta)

        gray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY) if src.ndim == 3 else src.copy()

        # Local smoothing reduces text noise / speckle edges
        gray = cv2.GaussianBlur(gray, (5, 5), 0)

        # Erode source to pre-close narrow gaps (keeps your intent)
        if gap_k > 1:
            kernel_gap = np.ones((gap_k, gap_k), np.uint8)
            gray = cv2.erode(gray, kernel_gap, iterations=1)

        # --- Edges -> walls ---
        # Thresholds scaled by "Aggressiveness" isn't meaningful; keep stable edges
        edges = cv2.Canny(gray, 50, 150)

        # Thicken walls so tiny breaks are sealed
        wall_thick = max(2, int(round(gap_k * 0.6)))
        se_wall = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * wall_thick + 1, 2 * wall_thick + 1))
        walls = cv2.dilate(edges, se_wall, iterations=1)

        # Optional: close small gaps in the wall network
        walls = cv2.morphologyEx(walls, cv2.MORPH_CLOSE, se_wall, iterations=1)

        # --- Flood fill mask: 1=blocked, 0=free ---
        ff_mask = np.zeros((h + 2, w + 2), np.uint8)
        ff_mask[0, :] = 1
        ff_mask[-1, :] = 1
        ff_mask[:, 0] = 1
        ...
        # Treat walls as blocked in floodFill mask
        ff_mask[1:-1, 1:-1][walls > 0] = 1

        # Flood using mask-only; image argument is ignored for region result
        dummy = np.zeros_like(src)

        try:
            cv2.floodFill(
                dummy, ff_mask, (int(sx), int(sy)), (255, 255, 255),
                (0, 0, 0), (0, 0, 0),
                flags=cv2.FLOODFILL_MASK_ONLY | cv2.FLOODFILL_FIXED_RANGE
            )
        except Exception:
