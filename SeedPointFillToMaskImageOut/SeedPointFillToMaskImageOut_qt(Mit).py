...
class FloodFillApp(QMainWindow):
    def __init__(self):
        ...
        self.extract_button.clicked.connect(self.extract_distances_along_contour)
    ...
    def extract_distances_along_contour(self):
        """
        Extracts oriented ROIs along the simplified contour, performs OCR, finds distance candidates,
        sums the total, updates the GUI, and saves results for future processing.
        """
        # 1. Create ROIs along the contour
        rois = self.ROI_create()

        # 2. Perform OCR and extract distance candidates
        ocr_results = self.OCR_find(rois)

        # 3. Sum distances and update GUI
        total_distance, all_distances = self.Distance_find(ocr_results)

        # 4. Optionally, visualize ROI boxes
        self.ocr_candidate_boxes = [r["roi_box"] for r in ocr_results]
        self.update_canvas_image()

        # 5. Display results in the table at bottom right (QTextEdit)
        self.table_text.clear()
        if ocr_results:
            for idx, r in enumerate(ocr_results):
                self.table_text.append(f"ROI {idx}: OCR='{r['ocr_text']}' Distances={r['distances']}")
            self.table_text.append(f"\nTotal Distance: {total_distance:.2f}")
        else:
            self.table_text.append("No distances found along contour lines.")

        # 6. Save for future processing
        self.distance_candidates = all_distances

        return ocr_results
    
    def ROI_create(self, contour=None, line_width=100):
        """
        Create oriented ROIs along a contour.
        Returns: list of (roi_img, roi_box) tuples.
        """
        if contour is None:
        ...
        for i in range(len(contour)):
            ...
            dst_rect = np.array([
                [0, 0],
                [roi_length - 1, 0],
                [roi_length - 1, roi_width - 1],
                [0, roi_width - 1]
            ], dtype=np.float32)
            M = cv2.getPerspectiveTransform(corners, dst_rect)
            roi_img = cv2.warpPerspective(img, M, (int(roi_length), int(roi_width)))
            rois.append((roi_img, np.int32(corners)))
        return rois

    def OCR_find(self, rois):
        """
        Preprocess and OCR each ROI.
        Returns: list of dicts with keys: 'roi_box', 'ocr_text', 'distances'
        Only keeps numbers, periods, and apostrophes in found text.
        Also saves each ROI image to disk for quality confirmation.
        """
        results = []
        export_dir = "roi_exports"
        if not os.path.exists(export_dir):
        ...
        for idx, (roi_img, roi_box) in enumerate(rois):
            # Save ROI image
            roi_filename = os.path.join(export_dir, f"roi_{idx}.png")
            cv2.imwrite(roi_filename, roi_img)

            proc = self.preprocess_roi_for_ocr(roi_img, idx)
            # roi = roi_img
            # OCR: extract text from ROI (normal)
            # ocr_text = pytesseract.image_to_string(proc, config="--psm 7 -c tessedit_char_whitelist=0123456789.'\"").strip()
            ocr_text = pytesseract.image_to_string(proc, config="--psm 6").strip()
            # OCR: extract text from ROI (inverted)
            roi_inverted = cv2.rotate(proc, cv2.ROTATE_180)
            # ocr_text_inverted = pytesseract.image_to_string(roi_inverted, config="--psm 7 -c tessedit_char_whitelist=0123456789.'\"").strip()
            ...
            print(f"Contour {idx} OCR candidates: {number_matches}")
            ...
            # ocr_text_raw = pytesseract.image_to_string(proc, config="--psm 6").strip()
            # # Only keep numbers, periods, and apostrophes
            # ocr_text = re.sub(r"[^0-9\.\']", "", ocr_text_raw)
            # candidates = re.findall(r"[^\s]*\d{1,4}(?:\.\d{1,3})?\'?[^\s]*", ocr_text)
            # print(f"ROI {idx} OCR candidates: {candidates}")  # <-- Print candidates here
            # distances = []
