...
class FloodFillApp(QMainWindow):
    ...
    def choose_gap_kernel_descending(self, start_k: int, perimeter_jump_ratio: float = 1.20, min_k: int = 1):
        """
    ...
    def flood_fill_and_show_mask_contours_modular(self):
        ...
        # Optionally log (kept out of UI change to slider)
        if gap_px != user_gap:
        ...
        new_region = ff_mask[1:-1, 1:-1]
        if np.count_nonzero(new_region) == 0:
            print("Flood produced no region.")
            return
        new_region_u8 = (new_region * 255).astype(np.uint8)
